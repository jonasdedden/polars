...
class _Provider:
    ...
    def __init__(self, *, wants_predicate: bool) -> None:
        self.calls: list[dict[str, Any]] = []
        self.wants_predicate = wants_predicate
    ...
    def _scan(self, kwargs: dict[str, Any]) -> tuple[pl.LazyFrame, str]:
        ...
        schema = SCHEMA if projection is None else pa.schema(
            [SCHEMA.field(name) for name in projection]
        )
    ...
    def to_dataset_scan(self, **kwargs: Any) -> tuple[pl.LazyFrame, str]:
        if not self.wants_predicate:
            assert "serialized_predicate" not in kwargs
...
class _NarrowProvider(_Provider):
    ...
    def __init__(self) -> None:
        super().__init__(wants_predicate=False)
...
class _PredicateProvider(_Provider):
    ...
    def __init__(self) -> None:
        super().__init__(wants_predicate=True)
...
def test_kwargs_provider_receives_the_predicate() -> None:
    provider = _Provider(wants_predicate=True)
